...
MainWindow::MainWindow ( QWidget *parent )
    ...
    if ( QLibrary::isLibrary ( "SerialPorts.dll" ) ) {
        ...
        if ( library.isLoaded() ) {
            serialPorts = ( EnumerateSerialPorts ) library.resolve ( "enumerateSerialPorts" );
            serialDll = true;
        }
    }
    ...
    if ( !serialDll ) {
        ...
        warningMessage.setText ( QObject::tr ( "Serial interfaces is not available." ) );
        warningMessage.setInformativeText ( QObject::tr ( "The application cannot use serial driver because the dll package is not available." ) );
...
void MainWindow::onPortSettings() {
    ...
    if ( !std::wcslen ( serial ) ) {
        ...
        warningMessage.setInformativeText ( QObject::tr ( "The application did not detect any serial interface on this computer." ) );
        warningMessage.setWindowFlags ( Qt::Dialog | Qt::CustomizeWindowHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint );
        warningMessage.exec();
        ui->rbManual->setChecked ( true );
    } else {
