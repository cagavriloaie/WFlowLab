...
void Dialog::onOpenInputDataClicked()
{
    QString fileName = QFileDialog::getOpenFileName(
                           this, \
                           tr("Open Input Data"),
                           QString(mainwindow->selectedInfo.pathResults.c_str()) + \
    ...
    mainwindow->ui->leTemperature->setText(std::to_string(
            ambientTemperature).c_str());
    mainwindow->ui->leHumidity->setText(std::to_string(
                                            relativeAirHumidity).c_str());
    mainwindow->ui->lePressure->setText(std::to_string(
                                            athmosphericPressure).c_str());
    if (rbVolumetric)
    {
...
std::string precision_2(double number)
{
    ...
    {
        return std::to_string((int) number) + ".0" + std::to_string(
                   decimal_part);
    }
}
...
void Dialog::ValidatorInput()
{
    ...
    for (auto iter = begin(vectorSerialNumber);
            iter != end(vectorSerialNumber); ++iter)
    {
    ...
    paletteDiactivatedLineEdit.setColor(QPalette::Base, QColor(220, 235,
                                        220, 255));
...
void Dialog::onCalculateClicked()
{
    ...
    if (mainwindow->selectedInfo.rbGravitmetric == true)
    {
        ...
        densityFirst = quadraticInterpolationTemperature(temperatureFirst,
                       correction);
        densitySecond = quadraticInterpolationTemperature(temperatureSecond,
                        correction);
        densityThird = quadraticInterpolationTemperature(temperatureThird,
                       correction);
    ...
    if (!result)
    {
        ...
        messageInputData.setText(
            tr("Some user input data are not available or are outside the normal rage!"));
...
void Dialog::onCbClicked(bool checked)
{
    ...
    auto iter = std::find(vectorCheckNumber.begin(),
                          vectorCheckNumber.end(), checkBox);
...
void Dialog::onPrintPdfDocClicked()
{
    ...
    QString minimumFlowString = to_string_with_precision(minimumFlow,
                                2).c_str();
    QString transitionFlowString = to_string_with_precision(trasitionFlow,
                                   2).c_str();
    QString nominalFlowString = to_string_with_precision(nominalFlow,
                                2).c_str();
    QString maximumFlowString = to_string_with_precision(maximumFlow,
                                2).c_str();
    ...
    if (mainwindow->selectedInfo.selectedLanguage == ROMANIAN)
    {
        report =
            ...
            "Temperatura:&nbsp;" + to_string_with_precision(ambientTemperature,
                    2).c_str() + "&nbsp;[°C]<br>" +
            "Presiune atmosferica:&nbsp;" + to_string_with_precision(
                athmosphericPressure, 2).c_str() + "&nbsp;[Pa]<br>" +
            "Umiditate:&nbsp;" + to_string_with_precision(humidity,
                    2).c_str() + "&nbsp;[%]<br><br>" +
            "Tip contor apa:&nbsp;" + nameWaterMeter + "<br>" +
            "Medoda de verificare:&nbsp;" + methodMeasurement + "<br>" +
            "Diametru nominal:&nbsp;" + to_string_with_precision(nominalDiameter,
                    2).c_str() + "&nbsp;[mm]<br>" +
            "Debit maxim:&nbsp;&nbsp;&nbsp;&nbsp;" +
            maximumFlowString + "&nbsp;[L/h]"  + "&nbsp;/&nbsp;" +
            "Max error:&nbsp;" +
            to_string_with_precision(nominalError,
                                     2).c_str() + "&nbsp;[%]" + "<br>" +
            "Debit nominal:&nbsp;&nbsp;&nbsp;&nbsp;" +
            ...
            "Max error:&nbsp;" +
            to_string_with_precision(maximumError,
                                     2).c_str() + "&nbsp;[%]" + "<br>" +
            "</h3>";
        report +=
                ...
                ++iter)
        {
            ...
            QString realVolumeFirst = precision_2(
                                          ui->leVolume1->text().toDouble()).c_str();
            ...
            QString realVolumeSecond = precision_2(
                                           ui->leVolume2->text().toDouble()).c_str();
            ...
            QString realVolumeThird = precision_2(
                                          ui->leVolume3->text().toDouble()).c_str();
            if (
            ...
            try
            {
                bool bFirst = std::abs(std::stod(errorFirst.toStdString().c_str())) <
                              maximumError;
                bool bSecond = std::abs(std::stod(errorFirst.toStdString().c_str())) <
                               nominalError;
                bool bThird = std::abs(std::stod(errorFirst.toStdString().c_str())) <
                              nominalError;
            ...
            report +=
                QString("    <tr>") +
                "        <th style=\"text-align: left\" rowspan=\"3\"><br>"   + SN +
                "</th>" +
    ...
    else
    {
        report =
            ...
            "Temperature:&nbsp;" + to_string_with_precision(ambientTemperature,
                    2).c_str() + "&nbsp;[°C]<br>" +
            "Athmopheric pressure:&nbsp;" + to_string_with_precision(
                athmosphericPressure, 2).c_str() + "&nbsp;[Pa]<br>" +
            "Humidity:&nbsp;" + to_string_with_precision(humidity,
                    2).c_str() + "&nbsp;[%]<br><br>" +
            "Water meter type:&nbsp;" + nameWaterMeter + "<br>" +
            "Used measurement method:&nbsp;" + methodMeasurement + "<br>" +
            "Nominal diameter&nbsp;:" + to_string_with_precision(nominalDiameter,
                    2).c_str() + "&nbsp;[mm]<br>" +
            "Maximum flow:&nbsp;&nbsp;&nbsp;&nbsp;" +
            maximumFlowString + "&nbsp;[L/h]"  + "&nbsp;/&nbsp;" +
            "Max error:&nbsp;" +
