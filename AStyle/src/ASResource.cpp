...
//
    ...
    const string ASResource::AS_AUTORELEASEPOOL =
    ...
    const string ASResource::AS_MODULE =
        string("module");  // CORBA IDL module definition
    ...
    const string ASResource::AS_REINTERPRET_CAST =
    ...
    const string ASResource::AS_LAMBDA =
        string("=>");            // C# lambda expression arrow
     ...
     */
    void ASResource::buildAssignmentOperators(vector<const string *>
            *assignmentOperators)
    {
        ...
        sort(assignmentOperators->begin(), assignmentOperators->end(),
             sortOnLength);
    }
     ...
     */
    void ASResource::buildCastOperators(vector<const string *>
                                        *castOperators)
    {
     ...
     */
    void ASResource::buildIndentableHeaders(vector<const string *>
                                            *indentableHeaders)
    {
...
//  sort(indentableHeaders->begin(), indentableHeaders->end(), sortOnName);
    ...
    */
     ...
     */
    void ASResource::buildNonAssignmentOperators(vector<const string *>
     ...
     */
    void ASResource::buildNonParenHeaders(vector<const string *>
                                          *nonParenHeaders, int fileType, bool beautifier)
    {
        ...
        nonParenHeaders->emplace_back(
            &AS_CATCH);       // can be paren or non-paren
        nonParenHeaders->emplace_back(
            &AS_CASE);        // can be paren or non-paren
     ...
     */
    void ASResource::buildPreBlockStatements(vector<const string *>
            *preBlockStatements, int fileType)
    {
        ...
        sort(preBlockStatements->begin(), preBlockStatements->end(),
             sortOnName);
    }
     ...
     */
    void ASResource::buildPreCommandHeaders(vector<const string *>
                                            *preCommandHeaders, int fileType)
    {
        const size_t elements = 10;
        static bool reserved = false;
        if (!reserved)
        ...
        if (fileType == C_TYPE)
        {
            ...
            preCommandHeaders->emplace_back(
                &AS_SEALED);            // Visual C only
        ...
        sort(preCommandHeaders->begin(), preCommandHeaders->end(),
             sortOnName);
    }
     ...
     */
    void ASResource::buildPreDefinitionHeaders(vector<const string *>
            *preDefinitionHeaders, int fileType)
    {
        ...
        sort(preDefinitionHeaders->begin(), preDefinitionHeaders->end(),
             sortOnName);
    }
