...
//-----------------------------------------------------------------------------
     ...
     */
    void ASEnhancer::enhance(string &line, bool isInNamespace,
                             bool isInPreprocessor, bool isInSQL)
    {
    ...
    void ASEnhancer::convertSpaceIndentToForceTab(string &line) const
    {
        ...
        size_t tabCount = newSpaceIndentLength /
                          tabLength;     // truncate extra spaces
     ...
     */
    size_t ASEnhancer::findCaseColon(const string &line,
                                     size_t caseIndex) const
    {
        size_t i = caseIndex;
    ...
    * indent a line by a given number of tabsets
     ...
     */
    bool ASEnhancer::isBeginDeclareSectionSQL(const string &line,
            size_t index) const
    {
        string word;
        size_t hits = 0;
     ...
     * @return              true if a hit.
     */
    bool ASEnhancer::isEndDeclareSectionSQL(const string &line,
                                            size_t index) const
    {
        string word;
        size_t hits = 0;
     ...
     */
    bool ASEnhancer::isOneLineBlockReached(const string &line,
                                           int startChar) const
    {
     ...
     */
    void ASEnhancer::parseCurrentLine(string &line, bool isInPreprocessor,
                                      bool isInSQL)
    {
        ...
        for (size_t i = 0; i < line.length(); i++)
        {
            ...
            {
                switchDepth++;
                switchStack.emplace_back(
                    sw);                      // save current variables
                sw.switchBraceCount = 0;
                sw.unindentCase =
                    false;                        // don't clear case until end of switch
    ...
    size_t ASEnhancer::processSwitchBlock(string &line, size_t index)
    {
        ...
        lookingForCaseBrace =
            false;                      // no opening brace, don't indent
        if (line[i] == '}')
        {
            sw.switchBraceCount--;
            if (sw.switchBraceCount ==
                    0)                 // if end of switch statement
            {
        ...
        if (isPotentialKeyword)
        {
            string name = getCurrentWord(line,
                                         i);          // bypass the entire name
            i += name.length() - 1;
        }
        return i;
    }
