...
//-----------------------------------------------------------------------------
    ...
    void ASFormatter::init(ASSourceIterator *si)
    {
        ...
        parenStack->emplace_back(
            0);               // parenStack must contain this default entry
        initContainer(braceTypeStack, new vector<BraceType>);
        braceTypeStack->emplace_back(
            NULL_TYPE);   // braceTypeStack must contain this default entry
    ...
    {
        if (getFileType() ==
                formatterFileType)  // don't build unless necessary
        {
            return;
        }
    ...
    void ASFormatter::fixOptionVariableConflicts()
    {
        ...
        else if (formattingStyle == STYLE_WHITESMITH)
        {
            ...
            setBraceIndent(true);
            setClassIndent(
                true);           // avoid hanging indent with access modifiers
            setSwitchIndent(
                true);          // avoid hanging indent with case statements
        }
        else if (formattingStyle == STYLE_VTK)
        {
            ...
            setBraceFormatMode(BREAK_MODE);
            setBraceIndentVtk(
                true);        // sets both braceIndent and braceIndentVtk
            setSwitchIndent(
                true);          // avoid hanging indent with case statements
        }
    ...
    string ASFormatter::nextLine()
    {
        ...
        while (!isLineReady)
        {
            ...
            {
                if (previousNonWSChar == '>' && isWhiteSpace(currentChar)
                        && peekNextChar() == '>')
                {
                    continue;
                }
                    ...
                    && !isBraceType(braceTypeStack->back(), SINGLE_LINE_TYPE))
            {
                        ...
                        && sourceIterator->tellg() > preprocBlockEnd)
                {
                    ...
                    string preproc = ASBeautifier::extractPreprocessorStatement(
                                         currentLine);
                    if (preproc.length() >= 2
                            && preproc.substr(0, 2) == "if") // #if, #ifdef, #ifndef
                    {
                        ...
                        isIndentableProprocessorBlock = isIndentablePreprocessorBlock(
                                                            currentLine, charNum);
                    ...
                    && methodAttachLineNum == 0)
            {
                if ((size_t) charNum == methodAttachCharNum)
                {
                    ...
                    int itemAlignment = (previousNonWSChar == '*'
                                         || previousNonWSChar == '^')
            ...
            // handle parens
            if (currentChar == '(' || currentChar == '[' || (isInTemplate
                    && currentChar == '<'))
            {
            ...
            else if (currentChar == ')' || currentChar == ']' || (isInTemplate
                     && currentChar == '>'))
            {
                foundPreCommandHeader = false;
                parenStack->back()--;
            ...
            if (currentChar == '{' || currentChar == '}')
            {
                ...
                if (currentChar == '}')
                {
                    ...
                    if (isNonInStatementArray
                            && (!isBraceType(braceTypeStack->back(),
                                             ARRAY_TYPE)    // check previous brace
                                || peekNextChar() ==
            ...
            if (isPotentialHeader && !isInTemplate && squareBracketCount == 0)
            {
                ...
                if (newHeader != nullptr)
                {
                            ...
                            && !isImmediatelyPostPreprocessor)
                    {
                        ...
                        size_t start = formattedLine.length() >= 6 ? formattedLine.length() -
                                       6 : 0;
                         ...
                         && !isInEnum)      // not C++11 enum class
                {
                    ...
                    if (newHeader == &AS_INTERFACE && !foundNamespaceHeader
                            && !foundClassHeader)
                    {
                ...
                if (isInCase)
                {
                    ...
                    if (shouldBreakOneLineStatements)
                    {
                        passedColon = true;
                         ...
                         && previousCommandChar !=
                         ')'  // not after closing paren of a method header
            ...
            if (isPotentialOperator)
            {
                ...
                if (newHeader != nullptr)
                {
                    ...
                    if (isInTemplate && (newHeader == &AS_GR_GR
                                         || newHeader == &AS_GR_GR_GR))
                    {
                        newHeader = &AS_GR;
                    }
                    if (!isInPotentialCalculation)
                    {
                        ...
                        if (find(begin(*assignmentOperators), end(*assignmentOperators),
                                 newHeader)
                                != end(*assignmentOperators))
                        {
                            foundPreCommandHeader = false;
                            char peekedChar = peekNextChar();
                            isInPotentialCalculation = !(newHeader == &AS_EQUAL
                                                         && peekedChar == '*')
            ...
            if (shouldPadOperators && newHeader != nullptr
            ...
            if (currentChar == '(' || currentChar == ')')
            {
                ...
                if (shouldPadParensOutside || shouldPadParensInside
                        || shouldUnPadParens || shouldPadFirstParen)
                {
                    padParens();
                }
                else
        ...
        bool isInNamespace = isBraceType(braceTypeStack->back(),
                                         NAMESPACE_TYPE);
                ...
                && previousReadyFormattedLineLength > 0)
        {
            ...
            enhancer->enhance(beautifiedLine, isInNamespace,
                              isInPreprocessorBeautify, isInBeautifySQL);
        }
...
// set objective-c method parameter type unpadding mode.
    ...
    */
    ...
    bool ASFormatter::getNextLine(bool emptyLineWasDeleted /*false*/)
    {
        ...
        {
            currentLine =
                "{";    // append brace that was removed from the previous line
        }
        else
        {
        ...
        {
            currentLine =
                string(" ");    // a null is inserted if this is not done
        }
        ...
        if (shouldDeleteEmptyLines
                && lineIsEmpty
                && isBraceType((*braceTypeStack)[braceTypeStack->size() - 1],
                               COMMAND_TYPE))
        {
            if (!shouldBreakBlocks || previousNonWSChar == '{'
    ...
    void ASFormatter::initNewLine()
    {
                ...
                && charNum + 1 < (int) len; charNum++)
        {
            ...
            {
                tabIncrementIn += tabSize - 1 - ((tabIncrementIn + charNum) %
                                                 tabSize);
            }
        }
        ...
        else if (isSequenceReached("{"))
        {
            ...
            if (firstText != string::npos)
            {
                         ...
                         || isExecSQL(currentLine, firstText))
                {
                    ...
                    size_t j;
                    for (j = charNum + 1; j < firstText
                            && isWhiteSpace(currentLine[j]); j++)
                    {
                        if (currentLine[j] == '\t')
                        {
     ...
     * @param canBreakLine     if true, a registered line-break
     */
    void ASFormatter::appendSequence(const string &sequence,
                                     bool canBreakLine)
    {
        if (canBreakLine && isInLineBreak)
        {
     ...
     * @param canBreakLine     if true, a registered line-break
     */
    void ASFormatter::appendOperator(const string &sequence,
                                     bool canBreakLine)
    {
        if (canBreakLine && isInLineBreak)
        {
...
//          || word == "UInt64"
    ...
    */
    ...
    bool ASFormatter::isDereferenceOrAddressOf() const
    {
        ...
        bool isDA = (!(isLegalNameChar(previousNonWSChar)
                       || previousNonWSChar == '>')
                     || (nextText.length() > 0 && !isLegalNameChar(nextText[0])
                         && nextText[0] != '/')
                     || (ispunct((unsigned char)previousNonWSChar)
                         && previousNonWSChar != '.')
                     || isCharImmediatelyPostReturn);
        return isDA;
    }
     ...
     */
    bool ASFormatter::isPointerOrReferenceVariable(const string &word)
    const
    {
        return (word == "char"
                || word == "int"
     ...
     */
    bool ASFormatter::isPointerToPointer(const string &line,
                                         int currPos) const
    {
     ...
     */
    int ASFormatter::isOneLineBlockReached(const string &line,
                                           int startChar) const
    {
    ...
    bool ASFormatter::isNextWordSharpNonParenHeader(int startChar) const
    {
        ...
        if (findKeyword(nextText, 0, AS_GET)
                || findKeyword(nextText, 0, AS_SET)
                || findKeyword(nextText, 0, AS_ADD)
                || findKeyword(nextText, 0, AS_REMOVE))
        {
            return true;
        }
        return false;
    ...
    */
    ...
    {
        assert(sourceIterator->getPeekStart() == 0
               || streamArg != nullptr);    // Borland may need != 0
...
//x                        && isPointerOrReference())
    ...
    void ASFormatter::formatPointerOrReferenceToMiddle()
    {
                     ...
                     || referenceAlignment == REF_SAME_AS_PTR))
        {
            sequenceToInsert = "*&";
            goForward(1);
            for (size_t i = charNum; i < currentLine.length() - 1
                    && isWhiteSpace(currentLine[i]); i++)
            {
                goForward(1);
            }
        }
        ...
        for (size_t i = charNum + 1; i < currentLine.length()
                && isWhiteSpace(currentLine[i]); i++)
        {
            goForward(1);
            if (formattedLine.length() > 0)
        ...
        size_t wsAfter = currentLine.find_first_not_of(" \t",
                         charNumSave + 1);
    ...
    void ASFormatter::formatPointerOrReferenceCast()
    {
        ...
        if (prevNum != string::npos)
        {
            ...
            if (itemAlignment == PTR_ALIGN_TYPE && currentChar == '*'
                    && prevCh == '*')
            {
    ...
    {
        assert(currentChar == '(' || currentChar == ')');
        assert(shouldPadParensOutside || shouldPadParensInside
               || shouldUnPadParens || shouldPadFirstParen);
            ...
            if (shouldUnPadParens)
            {
                ...
                if (i != string::npos)
                {
                    ...
                    else
                    {
                        ...
                        string prevWord = getPreviousWord(formattedLine,
    ...
    */
    void ASFormatter::padObjCParamType()
    {
        ...
        if (currentChar == '(')
        {
            ...
            size_t prevText = formattedLine.find_last_not_of(" \t",
                              paramOpen - 1);
...
//                  && peekNextChar() != '}'        // don't attach { }     // removed release 2.03
    ...
    void ASFormatter::formatClosingBrace(BraceType braceType)
    {
        ...
        if (attachClosingBraceMode)
        {
            ...
            if ((isEmptyLine(
                        formattedLine)         // if a blank line precedes this
                    || isCharImmediatelyPostLineComment
                    || isCharImmediatelyPostComment
                    || (isImmediatelyPostPreprocessor
                        && (int) currentLine.find_first_not_of(" \t") == charNum)
...
//              || (isBraceType(braceType, NAMESPACE_TYPE) && isOkToBreakBlock(braceType) && previousNonWSChar != '{')
     ...
     */
    void ASFormatter::formatArrayBraces(BraceType braceType,
                                        bool isOpeningArrayBrace)
    {
        assert(isBraceType(braceType, ARRAY_TYPE));
        assert(currentChar == '{' || currentChar == '}');
        if (currentChar == '{')
    ...
    {
        assert(braceFormatMode == RUN_IN_MODE
               || braceFormatMode == NONE_MODE);
        ...
        else if (getForceTabIndentation()
                 && getIndentLength() != getTabLength())
        {
            ...
            size_t tabCount = indent.length() /
                              tabLength_;     // truncate extra spaces
     ...
     */
    void ASFormatter::initContainer(vector<BraceType> *&container,
                                    vector<BraceType> *value)
    {
        if (container != nullptr)
        {
    ...
    */
     ...
     */
    const string *ASFormatter::checkForHeaderFollowingComment(
        const string &firstLine) const
    {
    ...
    {
        assert(currentChar == '#');
        const size_t preproc = currentLine.find_first_not_of(" \t",
                               charNum + 1);
        ...
        if ((doesLineStartComment
                ...
                && (shouldBreakElseIfs
                    ...
                    || (shouldBreakBlocks
                        ...
                        && previousCommandChar != '{')))
        {
            followingHeader = checkForHeaderFollowingComment(currentLine.substr(
                                  charNum));
        }
    ...
    void ASFormatter::formatCommentCloser()
    {
        ...
        goForward(1);
        if (doesLineStartComment
                && (currentLine.find_first_not_of(" \t",
                        charNum + 1) == string::npos))
        {
    ...
    void ASFormatter::formatQuoteBody()
    {
        ...
        else if (isInVerbatimQuote && currentChar == '"')
        {
            if (isCStyle())
            {
                ...
                if (delimStart > 0
                        && currentLine.substr(delimStart, delim.length()) == delim)
                {
                    isInQuote = false;
                    isInVerbatimQuote = false;
                }
    ...
    void ASFormatter::formatQuoteOpener()
    {
        ...
        if (isCStyle() && previousChar == 'R')
        {
            ...
            if (parenPos != -1)
            {
                ...
                verbatimDelimiter = currentLine.substr(charNum + 1,
                                                       parenPos - charNum - 1);
            }
        }
...
// for console build only
     ...
     */
    string ASFormatter::getPreviousWord(const string &line,
                                        int currPos) const
    {
        // get the last legal word (may be a number)
        if (currPos == 0)
    ...
    void ASFormatter::appendClosingHeader()
    {
        ...
        {
            previousLineIsOneLineBlock = isOneLineBlockReached(formattedLine,
                                         firstBrace);
        }
     ...
     */
    size_t ASFormatter::findNextChar(const string &line, char searchChar,
                                     int searchStart /*0*/) const
    {
        ...
        while (stream.hasMoreLines() || isFirstLine)
        {
            ...
            for (size_t i = 0; i < line.length(); i++)
            {
                ...
                if (line[i] == ';')
                {
                    ...
                    if ((shouldAttachReturnTypeDecl && foundSplitPoint
                            && isAlreadyBroken))
                    {
                        methodAttachCharNum = breakCharNum;
                        methodAttachLineNum = breakLineNum;
     ...
     */
    bool ASFormatter::isStructAccessModified(const string &firstLine,
            size_t index) const
    {
    ...
    */
    bool ASFormatter::isIndentablePreprocessorBlock(const string
            &firstLine, size_t index)
    {
        assert(firstLine[index] == '#');
        bool isFirstLine = true;
        ...
        bool isPotentialHeaderGuard =
            false;    // ifndef is first preproc statement
        bool isPotentialHeaderGuard2 =
            false;   // define is within the first proproc
        ...
        while (stream->hasMoreLines() || isFirstLine)
        {
            ...
            for (size_t i = 0; i < nextLine_.length(); i++)
            {
                ...
                if (blockContainsBraces || isInClassConstructor
...
EndOfWhileLoop:
    ...
    bool ASFormatter::isNDefPreprocStatement(const string &nextLine_,
            const string &preproc) const
    {
        if (preproc == "ifndef")
        {
            return true;
    ...
    {
        if (line[index] != 'e'
                && line[index] != 'E')   // quick check to reject most
        {
            return false;
        }
        string word;
    ...
    void ASFormatter::trimContinuationLine()
    {
        ...
        if (leadingSpaces > 0 && len > 0)
        {
            ...
            for (i = 0; (i < len)
                    && (i + continuationIncrementIn < leadingSpaces); i++)
            {
                ...
                if (currentLine[i] == '\t')
                {
                    continuationIncrementIn += tabSize - 1 - ((continuationIncrementIn +
                                               i) % tabSize);
                }
            }
    ...
    void ASFormatter::updateFormattedLineSplitPoints(char appendedChar)
    {
        ...
        {
            if (nextChar !=
                    ')'                     // space before a closing paren
                    && nextChar != '('              // space before an opening paren
                    && nextChar != '/'              // space before a comment
                    && nextChar != ':'              // space before a colon
                    && currentChar !=
                    ')'           // appended space before and after a closing paren
                    && currentChar !=
                    '('           // appended space before and after a opening paren
    ...
    void ASFormatter::updateFormattedLineSplitPointsOperator(
        const string &sequence)
    {
        ...
        if (sequence == "||" || sequence == "&&" || sequence == "or"
                || sequence == "and")
        {
            ...
            else
            {
                ...
                if (formattedLine.length() > sequenceLength
                        && isWhiteSpace(formattedLine[formattedLine.length() - sequenceLength
                                                                             - 1]))
                {
                    sequenceLength++;
                }
     ...
     */
    void ASFormatter::updateFormattedLineSplitPointsPointerOrReference(
        size_t index)
    {
        assert(maxCodeLength != string::npos);
        assert(formattedLine.length() > 0);
    ...
    void ASFormatter::testForTimeToSplitFormattedLine()
    {
        ...
        if (formattedLine.length() > maxCodeLength && !isLineReady)
        {
            ...
            if (splitPoint > 0 && splitPoint < formattedLine.length())
            {
                ...
                maxWhiteSpace = (maxWhiteSpace > splitPoint) ? (maxWhiteSpace -
                                splitPoint) : 0;
                if (maxSemiPending > 0)
                {
                    maxSemi = (maxSemiPending > splitPoint) ? (maxSemiPending -
                              splitPoint) : 0;
                    maxSemiPending = 0;
                }
                if (maxAndOrPending > 0)
                {
                    maxAndOr = (maxAndOrPending > splitPoint) ? (maxAndOrPending -
                               splitPoint) : 0;
                    maxAndOrPending = 0;
                }
                if (maxCommaPending > 0)
                {
                    maxComma = (maxCommaPending > splitPoint) ? (maxCommaPending -
                               splitPoint) : 0;
                    maxCommaPending = 0;
                }
                if (maxParenPending > 0)
                {
                    maxParen = (maxParenPending > splitPoint) ? (maxParenPending -
                               splitPoint) : 0;
                    maxParenPending = 0;
                }
                if (maxWhiteSpacePending > 0)
                {
                    maxWhiteSpace = (maxWhiteSpacePending > splitPoint) ?
                                    (maxWhiteSpacePending - splitPoint) : 0;
                ...
                else if (firstText > 0)
                {
                    ...
                    maxWhiteSpace = (maxWhiteSpace > firstText) ? (maxWhiteSpace -
                                    firstText) : 0;
                }
...
// The next word must be a legal name.
    const string *ASFormatter::getFollowingOperator() const
    {
        ...
        const string *newOperator = ASBase::findOperator(currentLine, nextNum,
                                    operators);
        return newOperator;
    }
...
// pad an Objective-C method colon
    void ASFormatter::padObjCMethodColon()
    {
                ...
                || nextChar == ')')
        {
            ...
            for (int i = formattedLine.length() - 1; (i > -1)
                    && isWhiteSpace(formattedLine[i]); i--)
            {
        ...
        {
            // pad space before
            for (int i = formattedLine.length() - 1; (i > 0)
                    && isWhiteSpace(formattedLine[i]); i--)
...
// Remove the leading '*' from a comment line and indent to the next tab.
    void ASFormatter::stripCommentPrefix()
    {
        ...
        if (isInCommentStartLine)
        {
            ...
            int followingText = formattedLine.find_first_not_of(" \t",
                                commentOpener + 2);
            if (followingText < 0)
            {
                return;
            ...
            if (formattedLine[followingText] == '*'
                    || formattedLine[followingText] == '!')
            {
                followingText = formattedLine.find_first_not_of(" \t",
                                followingText + 1);
            }
            if (followingText < 0)
            {
        ...
        if (formattedLine[firstChar] == '*')
        {
            ...
            else
            {
                ...
                int secondChar = formattedLine.find_first_not_of(" \t",
                                 firstChar + 1);
                if (secondChar < 0)
                {
                    adjustChecksumIn(-'*');
                ...
                else
                {
                    ...
                    formattedLine = string(spacesToInsert,
                                           ' ') + formattedLine.substr(secondChar);
                }
                // remove a trailing '*'
